"use client";

import { useEffect, useState } from "react";
import NavBar from "@/components/NavBar";
import { readCart, setQty } from "@/lib/cart";

export default function CheckoutPage() {
  const [summary, setSummary] = useState(null);
  const [err, setErr] = useState("");

  const load = async () => {
    setErr("");
    const cart = readCart();
    if (cart.length === 0) { setSummary({ lines: [], total: 0 }); return; }
    try {
      const res = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cart }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Checkout failed');
      setSummary(data);
    } catch (e) {
      setErr(e.message);
    }
  };

  useEffect(() => { load(); }, []);

  const onQty = async (id, qty) => {
    setQty(id, qty);
    await load();
  };

  return (
    <>
      <NavBar />
      <div className="bg-white rounded-xl p-6 shadow">
        <h1 className="text-xl font-semibold mb-4">Checkout</h1>
        {err && <div className="text-sm text-red-600 mb-3">{err}</div>}
        {!summary || summary.lines.length === 0 ? (
          <div className="text-gray-500 text-sm">Keranjang kosong. Tambahkan item di halaman Select.</div>
        ) : (
          <>
            <ul className="divide-y mb-4">
              {summary.lines.map(line => (
                <li key={line.id} className="py-3 flex items-center justify-between gap-3">
                  <div>
                    <div className="font-medium">{line.name}</div>
                    <div className="text-sm text-gray-600">Rp {line.price.toLocaleString('id-ID')}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <input
                      className="w-20 border rounded p-1 text-right"
                      type="number"
                      min={0}
                      defaultValue={line.qty}
                      onBlur={(e) => onQty(line.id, e.target.value)}
                    />
                    <div className="w-28 text-right font-medium">
                      Rp {line.lineTotal.toLocaleString('id-ID')}
                    </div>
                  </div>
                </li>
              ))}
            </ul>
            <div className="flex items-center justify-between">
              <div className="text-lg font-semibold">Total</div>
              <div className="text-lg font-semibold">Rp {summary.total.toLocaleString('id-ID')}</div>
            </div>
          </>
        )}
      </div>
    </>
  );
}
